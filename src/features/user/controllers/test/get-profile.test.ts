...
describe('Get', () => {
  ...
  describe('all', () => {
    it('should send success json response with users from database', async () => {
      const req: Request = authMockRequest({}, {}, authUserPayload, { page: '1' }) as unknown as Request;
      ...
      expect(userService.getAllUsers).toHaveBeenCalledWith(req.currentUser!.userId, 0, 12);
      expect(userService.getTotalUsersInDB).toHaveBeenCalled();
  ...
  describe('profile', () => {
    it('should send success json response with user from database', async () => {
  ...
  describe('profileAndPosts', () => {
    it('should send success json response with user and posts from database', async () => {
